...
export default class ApiClientBase{
    ...
    constructor(config: AxiosRequestConfig) {
        ...
        this.client.interceptors.response.use(<TResponse>(response: AxiosResponse<TResponse>)=>{
            ...
            (error: AxiosError)=> {
                return {
                    ...error,
                    data: new ApiResponse(null, error.response?.data as ProblemDetais, error.response?.status ?? 500)
                };
            }
            );
    }
    ...
    public async deleteAsync<T>(url: string, data?: any, config?: AxiosRequestConfig): Promise<ApiResponse<T>>{
